...
const CommercialFrameworkPage: React.FC<PageProps> = ({ proposal, pageNum }) => {
   ...
   const mCount = milestones.length;
    ...
    const circleSizeClass = 
       ...
       mCount <= 8 ? "w-9 h-9 text-[10px]" : 
       mCount <= 10 ? "w-7 h-7 text-[8px]" : "w-6 h-6 text-[7px]";

    const hideDesc = mCount >= 5;

    const titleTruncateClass = mCount <= 8 ? "whitespace-normal break-words line-clamp-2" : "truncate";
    const descTruncateClass = "whitespace-normal break-words line-clamp-2";

    const lineTopBottomClass = 
       mCount <= 3 ? "top-[33px] bottom-[33px]" :
       mCount <= 8 ? "top-[28px] bottom-[28px]" :
