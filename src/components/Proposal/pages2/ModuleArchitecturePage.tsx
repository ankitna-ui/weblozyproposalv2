...
import { CheckCircle2, IndianRupee, ChevronRight } from "lucide-react";
...
const ModuleArchitecturePage: React.FC<ModulePageProps> = ({ proposal, pageNum, pageSegments, pageIdx, totalPages }) => {
  return (
    <PageWrapper 
      pageNum={pageNum} 
      ...
      <div className="flex flex-col h-full overflow-visible">
        ...
        <div className="flex-1 flex flex-col gap-5 overflow-visible py-1">
          {pageSegments.map((segment, sIdx) => {
            const isFuture = segment.isFutureScalability;
            ...
            return (
            <div key={`${segment.id}-${sIdx}`} className={`module-card break-inside-avoid w-full border-[1px] ${borderColor} overflow-hidden rounded-lg shadow-sm`}>
              {/* Module Header */}
              {segment.isContinuation ? (
                 <div className={`${primaryColor} h-3 w-full border-b-[1px] ${borderColor}`} />
              ) : (
                   ...
                   <div className="flex items-center gap-2">
                     ...
                     {segment.price && (
                       <div className="bg-white px-2 py-0.5 rounded border border-slate-900 shadow-[1px_1px_0px_0px_rgba(0,0,0,1)] flex items-center gap-1">
                         ...
                         <span className="text-[11px] font-extrabold text-slate-900">{segment.price}</span>
                       </div>
                     )}
                   </div>
                 </div>
                ...
                {segment.features.map((feature: any, fIdx: number) => (
                  <div 
                    ...
                    className={`grid grid-cols-[1fr,auto,30px] items-center py-2.5 px-4 ${
                      fIdx !== segment.features.length - 1 ? "border-b border-slate-100" : ""
                    }`}
                  >
                    <div className="flex items-center gap-2">
